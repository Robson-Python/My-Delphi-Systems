          ...
          With dmCarroComando.ADOQuery1 do begin
            ...
            If vTotal > vLote then begin
               ...
               If StrToInt(vSaldo) = 0 then begin
                  ShowMessage('Esse lote já foi encerrado. Escolha o próximo lote.');
               end
               else begin
                  ShowMessage('A quantidade de animais da saída é maior que o saldo do lote selecionado! Você só pode vender '+vSaldo+' animais desse lote.');
               end;
            end
            else begin
               Active := False;
...
//          Application.MessageBox('Esse lote não possui saídas.','Omega - Aviso',MB_OK+MB_ICONINFORMATION);
//       end
//       else begin
          Active := False;
          SQL.Clear;
