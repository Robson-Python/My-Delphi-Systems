...
begin
    If edtConta.Text <> '' then begin
       ...
       Application.MessageBox('O campo Veículo não pode estar vazio!','MyCar - Atenção',MB_OK+MB_ICONWARNING);
    end;
end;
...
begin
    If edtCod.Text <> '' then begin
       ...
       Application.MessageBox('Selecione um veículo para atualizar dados.','MyCar - Atenção',MB_OK+MB_ICONWARNING);
    end;
end;
...
begin
    If edtCod.Text <> '' then begin
       ...
       Application.MessageBox('Selecione um veículo para poder excluir.','MyCar - Atenção',MB_OK+MB_ICONWARNING);
    end;
end;
    ...
    If cbxPeriodo.Text <> '' then begin
       If rdgTipo.ItemIndex <> -1 then begin
          If rdgSituacao.ItemIndex <> -1 then begin
             ...
             Application.MessageBox('Selecione a Situação da Conta para consultar!','MyCar - Atenção',MB_OK+MB_ICONWARNING);
             rdgSituacao.Buttons[0].SetFocus;
          end;
       end
       else begin
          Application.MessageBox('Selecione o Tipo de conta para consultar!','MyCar - Atenção',MB_OK+MB_ICONWARNING);
          rdgTipo.Buttons[0].SetFocus;
       end;
    end
    else begin
       Application.MessageBox('Selecione o Período/Mês para poder consultar!','MyCar - Atenção',MB_OK+MB_ICONWARNING);
       cbxPeriodo.SetFocus;
    end;
...
begin
    ...
    If rdgSituacao.ItemIndex = 1 then begin
       If dtpVencimento.Date < Date then begin
          Application.MessageBox('Essa conta está vencida.','MyCar - Atenção',MB_OK+MB_ICONINFORMATION);
...
begin
    If cbxPeriodo.Text <> '' then begin
       ...
       Application.MessageBox('Selecione o Período/Mês para imprimir!','MyCar - Atenção',MB_OK+MB_ICONWARNING);
    end;
end;
