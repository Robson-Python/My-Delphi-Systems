...
uses Principal_frm, DateUtils, Ativar_frm, SHELLAPI, Carro_Comando_dm;
...
begin
    ...
    With dmCarroComando.zqMyCar do begin
       Active := False;
       SQL.Clear;
       SQL.Add('Select * from mycar where codigo = 1');
       Active := True;
...
begin
    ExecFile('C:\Arquivos de programas\MyCar\Contrato MyCar (EULA).pdf');
end;
...
begin
    WinExec(PCHAR('C:\Arquivos de programas\Internet Explorer\iexplore.exe http://sites.google.com/site/mycarsoftware'),SW_SHOWMAXIMIZED);
end;
