...
begin
    With dmVendas.ADOQuery1 do begin
       Active := False;
       SQL.Clear;
       SQL.Add('Select itens_saida.*, replace(format(is_vl_uni,2), ".",",") ''uni'', replace(format(is_vl_tot,2), ".",",") ''tot'', produtos.* from itens_saida, produtos where itens_saida.p_cod = produtos.p_cod and sv_cod = '+QuotedStr(frmSelSaida.edtCod.Text)+' order by is_cod');
       Active := True;
