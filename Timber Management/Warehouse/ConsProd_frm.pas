...
begin
    With dmVendas.ADOQuery1 do begin
       Active := False;
       SQL.Clear;
       SQL.Add('Select itens_entrada.*, replace(format(itens_entrada.ie_vl_uni,2), ".",",") ''uni'', replace(format(itens_entrada.ie_vl_tot,2), ".",",") ''tot'', produtos.* from itens_entrada, produtos where itens_entrada.p_cod = produtos.p_cod and ec_cod = '+QuotedStr(frmSelEntrada.edtCod.Text)+' order by ie_cod');
       Active := True;
       ...
       DBGrid1.Columns.Items[4].FieldName := FieldByName('uni').FieldName;
       DBGrid1.Columns.Items[5].FieldName := FieldByName('tot').FieldName;
    end;
end;
