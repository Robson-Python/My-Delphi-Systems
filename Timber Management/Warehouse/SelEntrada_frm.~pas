    ...
    If CheckBox1.Checked = False then begin
       If rdbEntrada.Checked = True then begin
          With dmVendas.ADOQuery1 do begin
             Active := False;
             SQL.Clear;
             SQL.Add('select * from entrada, fornecedores where entrada.f_cnpj = fornecedores.f_cnpj order by ec_cod');
             Active := True;
             ...
             DBGrid1.Columns.Items[6].FieldName := FieldByName('ec_vl_entrada').FieldName;
       ...
       If rdbEntrada.Checked = True then begin
          With dmVendas.ADOQuery1 do begin
             Active := False;
             SQL.Clear;
             SQL.Add('select * from entrada, fornecedores where ec_dt_entrada between '+QuotedStr(DateToStr(DateTimePicker1.Date))+' and '+QuotedStr(DateToStr(DateTimePicker2.Date))+' and entrada.f_cnpj = fornecedores.f_cnpj order by ec_cod');
             Active := True;
             DBGrid1.Columns.Items[0].FieldName := FieldByName('ec_cod').FieldName;
             DBGrid1.Columns.Items[1].FieldName := FieldByName('f_cnpj').FieldName;
             DBGrid1.Columns.Items[2].FieldName := FieldByName('f_nome').FieldName;
...
begin
    With dmVendas.ADOQuery1 do begin
       Active := False;
       SQL.Clear;
       SQL.Add('select * from entrada, fornecedores where entrada.f_cnpj = fornecedores.f_cnpj and entrada.ec_n_nf = '+QuotedStr(edtNfiscal.Text)+' order by ec_cod');
       Active := True;
       DBGrid1.Columns.Items[0].FieldName := FieldByName('ec_cod').FieldName;
       DBGrid1.Columns.Items[1].FieldName := FieldByName('f_cnpj').FieldName;
       DBGrid1.Columns.Items[2].FieldName := FieldByName('f_nome').FieldName;
