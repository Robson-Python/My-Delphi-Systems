    ...
    If btnRegist.Caption = 'Ativar' then begin
       With dmVendas.ADOQuery1 do begin
          ...
          If Trim(nProtocolo) = Trim(edtLicenca.Text) then begin
             ...
             ExecSQL;
             Application.MessageBox('Sua Licença foi Ativada Para efetivar a Licença feche o Programa e execute-o novamente','Sistema Almoxarifado - Sucesso',MB_OK+MB_ICONINFORMATION);
             ShowMessage('O Nº do protocolo de ativação é: '+sProtocolo+'');
          end
          else begin
             Application.MessageBox('Licença Inválida! Entre em contato com o Programador para obter uma licença válida.','Sistema Almoxarifado - Erro Fatal',MB_OK+MB_ICONERROR);
          end;
       end;
    end
    else begin
       Application.MessageBox('Você possui uma licença válida.','Sistema Almoxarifado - Aviso',MB_OK+MB_ICONINFORMATION);
    end;
    Close;
end;
