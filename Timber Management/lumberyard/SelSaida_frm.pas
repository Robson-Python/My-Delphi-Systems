...
uses DB, Carro_Comando_dm, ADODB, Math, ConsProdSaida_frm;
    ...
    If CheckBox1.Checked = False then begin
       If rdbSaida.Checked = True then begin
          With dmVendas.ADOQuery1 do begin
             Active := False;
             SQL.Clear;
             SQL.Add('select * from saida, clientes where saida.c_cpfj = clientes.c_cpfj order by sv_cod');
             Active := True;
             ...
             DBGrid1.Columns.Items[4].FieldName := FieldByName('sv_tot_saida').FieldName;
       ...
       If rdbSaida.Checked = True then begin
          With dmVendas.ADOQuery1 do begin
             Active := False;
             SQL.Clear;
             SQL.Add('select * from saida, clientes where sv_dt_emiss between '+QuotedStr(DateToStr(DateTimePicker1.Date))+' and '+QuotedStr(DateToStr(DateTimePicker2.Date))+' and saida.c_cpfj = clientes.c_cpfj order by sv_cod');
             Active := True;
             DBGrid1.Columns.Items[0].FieldName := FieldByName('sv_cod').FieldName;
             DBGrid1.Columns.Items[1].FieldName := FieldByName('c_cpfj').FieldName;
             DBGrid1.Columns.Items[2].FieldName := FieldByName('c_nome').FieldName;
